...
```http
GET  /health              # провайдеры, имена в фильтре, names_dropped
GET  /v1/usage            # расход; ?user=имя — по одному человеку
POST /v1/redact           # что фильтр сделает с текстом (без вызова провайдера)
POST /v1/chat             # поля: provider, user
POST /v1/diagnostics/explain
GET  /v1/image/presets    # что реально можно попросить у картинок
POST /v1/image/generate   # картинка по тексту — персональных данных не уходит
POST /v1/image/edit       # ⚠️ отправляет РЕАЛЬНОЕ фото; см. предупреждение ниже
```

### 6б. Картинки — что действительно умеет провайдер

🔴 **Проверено прямым запросом 2026-08-10: GigaChat API НЕ редактирует фотографии.**

```
«Опиши, что на фото»   → «мужчина средних лет у окна…»            ✅ видит
«Отретушируй ЭТО фото» → «Я не могу редактировать изображения…»   ❌ не умеет
```

Работают два независимых шага: модель **смотрит** на фото и описывает его словами,
затем Kandinsky **рисует новое** по этому описанию. Лицо человека при таком пути
не сохраняется — это свойство пути, а не вопрос формулировки промпта.

| Пресет | Статус |
|---|---|
| `anime`, `cartoon`, `artistic` | ✅ «нарисуй по мотивам» — честное описание того, что произойдёт |
| `restore`, `colorize`, `glamour`, `upscale` | ⛔ убраны, возвращают `422` с объяснением |

Реставрация и ретушь с сохранением человека возможны **локально** — GFPGAN,
...
У каждого запроса есть поле `user`. Шлюз считает расход по каждому человеку отдельно
и хранит счётчики между перезапусками. Три независимых потолка, проверка **до** вызова:

1. личный дневной лимит (`LLM_USER_LIMITS="ulyana:5000 ivan:20000"`, иначе
   `LLM_USER_DAILY_TOKEN_LIMIT`);
2. дневной лимит на весь дом (`LLM_DAILY_TOKEN_LIMIT`);
3. месячный потолок по деньгам (`LLM_MONTHLY_COST_LIMIT_USD`).

`GET /v1/usage` отдаёт разбивку с остатком у каждого; `?user=имя` — по одному.

⚠️ Перерасход на один запрос возможен: стоимость ответа неизвестна заранее, поэтому
человек с лимитом 80 может закончить на 101. Следующий запрос уже отклоняется.
