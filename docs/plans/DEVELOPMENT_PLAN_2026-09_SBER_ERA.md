...
| D4 | Immich ML: Cloud.ru (ADR-0010, превью уходят из дома) **или** станция батчем по туннелю | станция батчем; Cloud.ru — только с принятием риска ПДн | **Cloud.ru, если укладывается в free tier** (риск ПДн принят при этом условии). Сначала спайк: E6 после E5. **Обновлено 2026-09-26:** решение — вариант Б (станция батчем); комплект готов в git (compose, `immich_ml_station.ps1`, `set_ml_url.sh`, `docs/plans/IMMICH_ML_STATION_RUNBOOK.ru.md`), **ждёт включения гипервизора на станции** (единственный блокер) | E6 |
...
**E** — Сбер-edge и ИИ · **F** — платформа · **G** — инженерный стандарт и чистка репозитория · **H** — фотоархив: поиск и воспоминания. В скобках — исходные ID (H* — GigaCode/`audit_new`,
...
### Этап H — семейный фотоархив: поиск и «воспоминания» (запрос владельца 2026-09-20)

Владелец: «хочу, чтобы бот присылал подборки по датам, как у Google и Яндекса — воспоминания», и чтобы
семье было проще искать в архиве 1,4 ТБ на HDD. Первый шаг — инвентаризация (идёт), она и определит развилку.

| ID | Задача | Условие/замер | Зависит от |
|---|---|---|---|
| H1 | **Каталог архива**: структура, объём, типы, EXIF (даты съёмки, камера, геометки), дубликаты, проблемные файлы. Подробности — вне git (`docs/local/`), в git — только агрегаты | запущено 2026-09-20 | — |
| H2 | **Развилка по итогам H1:** (А) подключить архив к Immich как внешнюю библиотеку — поиск, лица и воспоминания уже есть в Immich, бот берёт подборки через его API; (Б) свой лёгкий индекс (SQLite: путь, дата, тип) и подборки в боте. Решает владелец по числам: сколько снимков, сколько места займут превью, сколько выдержит Jetson | после H1 | H1 |
| H3 | **«Воспоминания» в боте**: раз в день/неделю подборка «N лет назад в этот день» в семейную группу; по запросу — «бобик, покажи лето 2019». Фото отдаются из дома (Samba/Nextcloud/Immich), наружу не уходят | после H2 | H2, Telegram-бот |
| H4 | **Поиск по архиву для семьи**: по дате и по папке — минимум; по смыслу и лицам — только если выбран путь (А) и решён E6 (Immich ML) | после H2 | H2, E6 |
| H5 | **Риск сохранности:** архив в одном экземпляре (владелец принял риск, B5). Любая индексация — только чтение; перед подключением к Immich проверить, что внешняя библиотека не пишет в исходные каталоги | до H2 | — |
