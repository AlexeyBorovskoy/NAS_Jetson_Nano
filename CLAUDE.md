...
| Vostro 15 (личная машина владельца) | **`192.168.75.177`** (чужая сеть) | Dell Vostro 15 (2018), Ubuntu 24.04, user `alexey`, SSH по ключу. L0 пройден 2026-08-11: 2 ядра, 3.7 ГБ, **825 ГБ свободно**, **CUDA нет**, исходящий TCP/22 на VPS работает. Роль пересмотрена: **off-site бэкап + внешний сторож**, не ML. Машину делит второй проект владельца → реестр занятого в `HOST_CONTRACT.md` **на самом ноутбуке**. См. `docs/plans/VOSTRO_ML_NODE_ONBOARDING.md` |
