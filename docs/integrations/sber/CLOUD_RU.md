...
🔴 **Поле `cost` — СТАВКА тарифа за единицу, а не начисленная сумма** (замер с устройства
2026-09-21, найдено соседним проектом). Деньги = `amount` × `cost`. Сумма столбца `cost` за
сентябрь даёт **953.34 ₽** при реально начисленных **7.72 ₽** — завышение в 123 раза, потому что
единица у токенов «млн шт»: 466.67 ₽ за МИЛЛИОН токенов при израсходованных 0.014 миллиона.
Признак без разбора: у входных и генерируемых токенов одинаковое 466.67 — совпадают ставки,
а не потребление. **Разделение полей, проверенное на живых данных:** `amount` — тарифицируемое
количество (деньги), `usefact` — фактическое потребление (бесплатный тариф). У Object Storage
`amount = 0` при `usefact` 0.19 ГБ и реальных операциях: по деньгам ноль, по тарифу расход виден.
Прежняя фраза «видно реальное потребление GigaChat-2-Max за сентябрь» формально верна, но рядом
с полем `cost` читается как «потрачено 933 ₽» — так её и прочли оба проекта.
