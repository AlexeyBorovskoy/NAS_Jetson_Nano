...
- 🔴 **Свежий код GigaCode в `main` ломает `@бобик` целиком** (CONFIRMED исполнением): в шлюзе
  `chat()` обращается к `full` до её определения → **каждый запрос к GigaChat (провайдер по
  умолчанию) отдаёт HTTP 500**; удалено `_IMG_TAG_RE`, которое ещё используется (генерация картинок
  упадёт с `NameError`); 2 существующих теста падают. Во время аудита правка перешла из staged в
  коммит `d52c11b` и запушена; pre-commit-ворота её пропустили, т.к. не запускают `tests/llm_gateway`.
  На устройство **не выкачено**. Это блокер любого деплоя `main` (Stage 0).
...
1. Не выкатывать `main`, пока `chat()` в шлюзе не исправлен и не прошёл весь набор тестов; добавить `tests/llm_gateway` в pre-commit-ворота.
...
|---|---|---|---|---|---|---|
| P0 | R0-01 | Исправить `chat()`/`_IMG_TAG_RE` в `main` (`d52c11b`), тесты шлюза — в pre-commit | не сломать `@бобик` | S | низкий | нет |
...
1. Исправить `d52c11b` в `main` (порядок `full`, вернуть `_IMG_TAG_RE`), добавить тест `POST /v1/chat` через `TestClient` и включить `tests/llm_gateway` в `preflight.sh`.
