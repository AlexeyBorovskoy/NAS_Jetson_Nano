...
🔴 СЕМАНТИКА ПОЛЕЙ (доказана замером 2026-09-21, 13 строк из 13):
    usefact    — количество в единице из `unit` (млн шт, ГБ, тыс. шт);
    cost       — СТАВКА тарифа, рублей за единицу;
    amount     — УЖЕ НАЧИСЛЕНО, рублей без НДС, ровно = usefact * cost;
    amount_nds — начислено с НДС, ровно = amount * 1.22.
Деньги = сумма `amount`. Ничего не умножать. Решающий довод — НДС: он лежит
на `amount`, а на количество НДС не начисляют.

⚠️ Хранилище — исключение, где это НЕ проверено: у всех строк Object Storage
`amount = 0` (уложились в бесплатный тариф), и произведение ничего не
подтверждает — ноль умножается на что угодно. Что именно значит `usefact` для
хранения (мгновенные ГБ? среднесуточные? ГБ·сутки?), **неизвестно**: замер
2026-09-21 показал 0.0656 (20.09) и 0.1250 (21.09) в тот день, когда в корзине
реально лежало 7.5 ГБ — расхождение в 60 раз. Поэтому сопоставление
FREE_TIER_LIMITS для Object Storage считать НЕПРОВЕРЕННЫМ: по такому
показанию 7.5 ГБ (половина бесплатного тарифа) выглядят как меньше процента.
Указано соседним проектом (доска, m0142).

🔴 ДВЕ ОШИБКИ ПОДРЯД, обе записаны, чтобы не повторить (2026-09-21):
  1. Сумма столбца `cost` = 953.34 ₽ — сложение СТАВОК. Неверно.
  2. Сумма `amount * cost` = 7.72 ₽ — умножение денег на ставку второй раз.
     Тоже неверно, и эту версию я успел выкатить и закрепить тестом.
  3. Верно: сумма `amount` = **0.0263 ₽** (0.0321 с НДС) за 01.08–21.09.
Первую ошибку нашёл сосед, вторую он же и отозвал через час, приведя
доказательство; проверено здесь независимо. Признак, который был у меня перед
глазами и не был использован: отношение `amount / usefact` у токенов даёт
ровно 466.67, то есть ставку — значит `amount` уже произведение.

Хуже завышения был второй дефект:
...
`amount`, бесплатный тариф — по `usefact`, **но для хранилища этот второй
вывод не проверен** (см. оговорку выше).
...
def summarize(consumptions):
    ...
    for item in consumptions:
        ...
        # Семантика полей, доказанная замером 2026-09-21 (13 строк из 13):
        #   usefact    — количество в единице из `unit` (млн шт, ГБ, тыс. шт);
        #   cost       — СТАВКА тарифа, рублей за единицу;
        #   amount     — УЖЕ НАЧИСЛЕНО, рублей без НДС, = usefact * cost;
        #   amount_nds — то же с НДС, = amount * 1.22.
        # Деньги берём из `amount` готовыми. Умножать нельзя — это была
        # ошибка первой починки, см. историю дефекта в docstring модуля.
        money = float(item.get("amount") or 0)
        money_nds = float(item.get("amount_nds") or 0)
        # ⚠️ Отката на `amount` здесь быть не должно: `amount` — деньги, а не
        # количество. Нет `usefact` — считаем количество неизвестным (0), но
        # деньги при этом остаются верными.
        usefact = float(item.get("usefact") or 0)
        unit = item.get("unit") or ""

        srow = by_service.setdefault(name, {"cost": 0.0, "cost_nds": 0.0,
                                            "usefact": 0.0,
                                            "unit": unit, "count": 0})
        srow["cost"] += money
        srow["cost_nds"] += money_nds
