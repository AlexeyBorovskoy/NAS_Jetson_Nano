...
🔴 **Деньги лежат в `amount` готовыми. Ни складывать ставки, ни умножать.**
Семантика доказана замером с устройства 2026-09-21 — 13 строк из 13:
...
Решающий довод — **НДС**: он лежит на `amount`. На количество НДС не начисляют, значит `amount` —
денежная величина. Три прочтения одной выгрузки за 01.08–21.09, и только третье верно:

| Как считали | Результат | Вердикт |
|---|---|---|
| сумма `cost` | 953.34 ₽ | сложение ставок — неверно |
| сумма `amount × cost` | 7.7222 ₽ | деньги на ставку второй раз — неверно, **успело попасть в код** |
| **сумма `amount`** | **0.0263 ₽** (0.0321 с НДС) | верно |

Признак, который стоял в собственной выгрузке и не был использован: `amount / usefact` у токенов
даёт ровно **466.67** — то есть ставку. Значит `amount` уже произведение. Ошибку нашёл соседний
проект (`m0138`), свою же формулу отозвал через час (`m0141`); обе версии проверены здесь
независимо. Прежняя фраза «видно реальное потребление GigaChat-2-Max за сентябрь» формально верна,
но рядом с полем `cost` читается как «потрачено 953 ₽» — так её и прочли оба проекта.

🟠 **Не проверено и важно: что значит `usefact` для Object Storage.** Равенство
`amount = usefact × cost` подтверждено только на токенах. У хранилища `amount = 0` (уложились в
бесплатный тариф), и произведение не проверяет ничего — ноль умножается на что угодно. В день,
когда в корзине реально лежало **7.5 ГБ**, биллинг показал `usefact` **0.0656** (20.09) и
**0.1250** (21.09) — расхождение в 60 раз. Мгновенные ГБ? среднесуточные? доля месяца? —
**неизвестно**. Пока не выяснено, правило «остаток бесплатного тарифа вычитаем по `usefact`»
для хранилища даёт **ложное спокойствие**: половина лимита выглядит как меньше процента.
Указано соседним проектом (`m0142`).
...
🔴 **2026-09-21 корзина очищена по решению владельца.** Удалено 462 объекта / 7.50 ГБ
(весь префикс `immich/` — репозиторий restic с оригиналами Immich и дампами БД),
повторная опись показала 0 объектов. Off-site (L2) копии фотографий больше нет.
✅ **P0 «один экземпляр на SSD» закрыт 2026-09-22 L1 HDD** (`CHECKPOINT_2026-09-22.md`), не S3.
Причина, названная при постановке, — освободить место соседнему проекту — **не подтвердилась**:
организация общая, но Object Storage соседу нужно ноль (доска, `m0137`). Решение подтверждено
владельцем повторно после предупреждения о цене.

🔴 **`restic` с Jetson: Stat `config/` → HTTP 400** (2026-09-21). Ключ в `.env` верный
(`AWS_ACCESS_KEY_ID` 69 симв., 1 двоеточие). Прямой SigV4 List/Delete с той же машины работал.
Рабочая гипотеза (ещё не прогнана на устройстве 2026-09-22 — туннель `:10022` не слушал):
Cloud.ru Object Storage = **path-style**; restic/minio по умолчанию может бить в
`bucket.s3.cloud.ru` → **400**. В скриптах: `-o s3.bucket-lookup=path`.
Проверка на Jetson: `sudo bash scripts/backup/restic_s3_cloudru_probe.sh`.
Даже при живом клиенте префикс `immich/` **пуст** после удаления 21.09 — без `restic init`
репозитория нет (это не 400, а отсутствие `config`).
**Вывод для следующей off-site копии:** восстановление проверять с той машины, которая будет
восстанавливать, а не с той, где удобнее.
