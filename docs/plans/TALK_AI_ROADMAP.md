...
|---|---|---|---|
| **A. Текстовый бот (MVP)** ✅ код готов | Фоновый опрос комнаты (polling, admin-OCS), команды `ping` / `статус` / `диск` / `фото` | `app/routers/talk_bot.py`, `TALK_BOT_ENABLED` | низкая |
...
| **C. Свободные вопросы** | Открытые вопросы уходят в DeepSeek через redaction-шлюз; function-calling для выбора «данные vs LLM» | LLM Gateway (готов) | средняя |
