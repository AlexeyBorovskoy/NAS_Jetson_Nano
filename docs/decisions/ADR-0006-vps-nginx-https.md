...
> ⚠️ **Обновление 2026-08-07 / Update 2026-08-07.** Адрес VPS сменился:
> `193.8.215.130` → **`95.163.176.103`** (прежний IP заблокирован российскими ISP).
> Кроме того, HTTPS-порты больше **не открыты в интернет** — ufw пускает их только с
> `172.29.172.0/24` и `10.8.1.0/24`, то есть доступ снаружи возможен только через VPN.
