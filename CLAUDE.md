...
- 🟠 **Исключение `192.168.0.0/16` в split tunneling Amnezia отрезает Vostro.** Маска, добавленная «чтобы видеть домашнюю сеть», покрывает и корпоративную `192.168.75.0/24`. Трафик к `192.168.75.177` уходит мимо туннеля в домашний Ethernet, где на этот адрес никто не отвечает. Проверено 2026-08-13: `ping` 100 % потерь, `ssh :22` timeout, `Find-NetRoute` → `InterfaceAlias: Ethernet, NextHop: 0.0.0.0, DestinationPrefix: 192.168.75.177/32` (адрес считается on-link). Отказ происходит **на рабочей станции**, до Vostro пакеты не доходят — ufw соседей ни при чём. Лечится точечным исключением вместо `/16`; решение за владельцем.
...
| Vostro 15 (личная машина владельца) | **`192.168.75.177`** (чужая сеть) | 🟠 **С рабочей станции сейчас недостижим** (2026-08-13) — см. граблю про `/16` в split tunneling. Dell Vostro 15 (2018), Ubuntu 24.04, user `alexey`, SSH по ключу. L0 пройден 2026-08-11: 2 ядра, 3.7 ГБ, **825 ГБ свободно**, **CUDA нет**, исходящий TCP/22 на VPS работает. Роль пересмотрена: **off-site бэкап + внешний сторож**, не ML. Машину делит второй проект владельца → реестр занятого в `HOST_CONTRACT.md` **на самом ноутбуке**. См. `docs/plans/VOSTRO_ML_NODE_ONBOARDING.md` |
