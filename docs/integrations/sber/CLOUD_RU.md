...
🔴 **Не закрыто: `restic` с устройства этот репозиторий не открывал.**
`Stat(<config/>) failed: Stat: 400 Bad Request` (17 повторов, `Fatal: unable to open config file`) —
при том, что прямой S3 подписью SigV4 с того же Jetson и того же ключа работает без единого отказа
(опись и удаление прошли), а `AWS_ACCESS_KEY_ID` в `config/.env` устройства — в правильной форме
`<tenant>:<keyId>`. Значит проверка восстановления 2026-09-20 выполнялась **не с устройства**.
Причина 400 не установлена; записано как неизвестное, а не как догадка про регион или path-style.
