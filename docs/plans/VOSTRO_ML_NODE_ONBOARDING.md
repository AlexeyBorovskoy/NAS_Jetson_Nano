...
> ⚠️ **Актуальные данные машины — 2026-08-13.** Всё, что ниже описано как
> «Ubuntu 24.04» и «`192.168.75.177`», относится к состоянию **до переустановки
> 11.08.2026** и оставлено как история решения. Сейчас:
>
> | | Было в этом документе | Есть на самом деле (замер 2026-08-13) |
> |---|---|---|
> | IP | `192.168.75.177` | **`192.168.75.153`** |
> | ОС | Ubuntu 24.04 | **Linux Mint 22.3 XFCE**, ядро 7.0.0-28 |
> | Доступ | `ssh alexey@…` | тот же, по ключу, **но только пока рабочая станция в `192.168.75.0/24`** |
>
> Живое состояние общей машины — `HOST_CONTRACT.md` **на самом ноутбуке**
> (728 строк), а не этот план и не локальная копия.
...
## 🔄 Что изменилось 2026-08-11 — L0 выполнен, роль пересмотрена

Живая проверка ноутбука проведена. Три результата меняют план.

### 1. ✅ Блокер L0 снят: исходящий TCP/22 на VPS работает

Проверено не пингом, а баннером — отвечает именно наш sshd:

```
95.163.176.103:22   → SSH-2.0-OpenSSH_9.6p1 Ubuntu-3ubuntu13.1   ✅
95.163.176.103:8090 → закрыт                                      (ufw, так и задумано)
pypi / github / huggingface → 200                                 ✅
```

Раздел 2 (двойной туннель) реализуем как написан. Порт `13003` на VPS свободен.

### 2. 🔴 Ловушка: `172.29.172.1` с ноутбука ведёт НЕ на наш VPS

На ноутбуке поднят клиент AmneziaWG (`awg0`, `10.8.1.26/32`), но **через него не
проложено ни одного маршрута** — весь трафик уходит через шлюз чужой сети. Итог:

```
172.29.172.1:22 с рабочей станции → OpenSSH_9.6p1   ← наш VPS
172.29.172.1:22 с этого ноутбука  → OpenSSH_8.9p1   ← ПОСТОРОННЯЯ машина
```

Запасной адрес из CLAUDE.md **с этой машины использовать нельзя** — один и тот же
адрес указывает на разные хосты в зависимости от того, откуда смотришь. Только
публичный `95.163.176.103`.

### 3. 🟠 Машина не свободна и роль ML под вопросом

Ноутбук — **личный компьютер владельца**, задействовать разрешено полностью. Но на
нём уже работает второй проект владельца (веб-сервер, СУБД и приложение), и туда же
готовится постоянный сервис, который должен отвечать людям без задержек.

Отсюда пересмотр: **ML-роль (раздел 2) конфликтует со вторым проектом по процессору.**
Два ядра без turbo — это ресурс, который нельзя занять наполовину. Immich ML на CPU
съест его целиком на несколько ночей подряд.

| Роль | Сильная сторона машины | Вердикт |
|---|---|---|
| **Off-site бэкап restic** | 825 ГБ + **другая сеть, другое здание** | ✅ брать первой |
| **Внешний сторож** (видит пропажу домашнего интернета) | расположение вне дома | ✅ дёшево, брать |
| Immich ML на CPU | — | 🟠 отложить: отнимает CPU у соседа |
| Любой локальный LLM | — | ⛔ 2 ядра / 3.7 ГБ — бессмысленно |

⚠️ **Открытый вопрос владельцу:** off-site бэкап — это семейные фото и БД на машине
в чужой сети. Restic шифрует репозиторий, посторонний увидит только непрозрачные блобы.
Решение за владельцем; **до явного согласия не разворачивать.**

### Согласование с соседним проектом

Общий ресурс — не только ноутбук, но и **VPS**: его порты уже конфликтуют. Оба проекта
ведут единый реестр занятого — файл `HOST_CONTRACT.md` **на самом ноутбуке**
(копия вне git: `docs/local/VOSTRO_HOST_CONTRACT.md`). Прямого канала между агентами
двух проектов нет, поэтому источником истины назначен файл на общей машине.

**Занято проектом NAS на VPS:** `10022`, `12283`, `18080`, `18090`, `18099`, `45876`,
`62822`. Планируется `13003` (Immich ML, если роль вернётся).

---

## 1. Железо / Hardware

> 🇷🇺 Подтверждено живым замером на машине 2026-08-11 (L0). Сетевые параметры —
> локальный источник вне git, см. `docs/local/`.
...
| Сеть / Network | ~~`192.168.75.177`~~ → **`192.168.75.153`** (корпоративная LAN); Wi-Fi 802.11ac 1×1 (QCA9377) |
...
### L0 — Снять живое состояние / Capture live state ✅ ВЫПОЛНЕН 2026-08-11

> Результаты — в блоке «Что изменилось 2026-08-11» выше. Кратко: исходящий TCP/22
> на VPS **проходит**, стенда ZTN на машине **уже нет** (`virsh` не установлен,
> процессов QEMU нет), свободно 825 ГБ, CUDA нет. Команда ниже оставлена для повтора.
>
> ⚠️ **Грабля при повторе:** `ffmpeg` и другие команды, читающие stdin, съедают
> остаток скрипта, переданного через `ssh "bash -s"`. Добавлять `-nostdin`
> или `</dev/null`. Тот же класс ошибки, что и `sudo -S` с heredoc.

```bash
ssh alexey@192.168.75.153 "   # ⚠️ было .177 до переустановки 11.08
...
### L1 — Освободить ресурсы от стенда ZTN ✅ УЖЕ НЕ ТРЕБУЕТСЯ

> Проверено 2026-08-11: `virsh` на машине **не установлен**, процессов `qemu-system`
> нет, `libvirtd` отсутствует. Стенд снят до нас — освобождать нечего.
> Блок ниже оставлен на случай, если стенд когда-нибудь вернётся.
...
### L2 — Работа 24/7 с закрытой крышкой ⬅️ СЛЕДУЮЩИЙ ШАГ

> Проверено 2026-08-11: сейчас `sleep-inactive-ac-type = 'suspend'`, крышка закрыта →
> сон. **Это ломает обе роли машины сразу** — и нашу, и соседнего проекта, которому
> нужен сервис, переживающий перезагрузку. Изменение общее, делается один раз,
> отмечается в `HOST_CONTRACT.md` на самом ноутбуке.
...
- `HOST_CONTRACT.md` **на самом ноутбуке** — реестр того, кто из двух проектов что
  занял на машине и на VPS. Локальная копия: `docs/local/VOSTRO_HOST_CONTRACT.md`
- `docs/local/` — сетевые параметры ноутбука и всё, что не идёт в публичный репозиторий
