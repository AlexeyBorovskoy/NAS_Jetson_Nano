...
>
> Updated / Обновлено: **2026-09-12** (mesh Deco перестроен, замеры пересняты).

## 1а. Current measured state / Текущее измеренное состояние (2026-09-12)

🇷🇺 Блок переписан по живым замерам 12.09.2026. Прежние цифры (2026-08-10, одна точка
`TP-Link_828C`, канал 48, сигнал 48 %) относились к сети **до** ввода mesh и удалены,
чтобы не выдавать историю за текущее состояние. Полный разбор дня —
[`34_NETWORK_SNAPSHOT_2026-09-12.md`](34_NETWORK_SNAPSHOT_2026-09-12.md).

🇬🇧 Rewritten from live measurements on 2026-09-12. The previous figures described the
pre-mesh network and were removed rather than kept alongside.
...
| Deco WAN / LAN | `192.168.0.107` / `192.168.68.1`, подсеть `192.168.68.0/**22**` | ARP, `Get-NetIPAddress` |
| Адрес дочернего узла | **плавает**: `.250 → .249 → .250` за один день | ARP после перезагрузок |
| SSID | единый `mash`, 5 ГГц, канал 40 | `netsh wlan show interfaces` |
| Рабочая станция | 5 ГГц, **866.7 Мбит/с**, сигнал **85 %** | `netsh wlan` |
| Wi-Fi → интернет | **100 Мбит/с** | `curl`, 2 прогона |
| Провод → интернет | **87–92 Мбит/с** | `curl` с Jetson, 3 прогона |
| Через AmneziaVPN | 46–53 Мбит/с, 1 прогон из 4 сорвался до 0.8 | `curl`, вперемежку с обходом |
| Удержание сессии 20 мин | через VPN и мимо — **обе дожили**, обрывов нет | `curl --limit-rate 50k` |
| DNS через Deco / EC220 | **7 мс** / 5 мс | `Resolve-DnsName` |
| Пинг до узлов / Jetson | 1.3 мс / 0.4 мс / 2.3 мс | 30 пакетов, потерь нет |

**Вывод / Conclusion:** узкое место больше не Wi-Fi. Радиоканал (866 Мбит/с) и провод
(гигабит) перекрывают канал провайдера (≈ 90–100 Мбит/с) с запасом. Прежний вывод
«одна точка квартиру не покрывает» закрыт вводом mesh; цена вопроса — правильное
размещение второго узла, см. слепок 34.

⚠️ **EC220 продолжает раздавать собственный Wi-Fi** параллельно с mesh и делит с ним
эфир. Решение не принято. / The EC220 still broadcasts its own Wi-Fi alongside the mesh.
