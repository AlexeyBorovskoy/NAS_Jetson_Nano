...
> ⚠️ **Обновление 2026-08-07 / Update 2026-08-07.** Решение в силе, но **адрес VPS сменился**:
> `193.8.215.130` → **`95.163.176.103`** (прежний IP заблокирован российскими ISP).
> Адреса ниже по тексту документа приведены на момент принятия решения.
> Хост туннеля задаётся переменной `VPS_HOST` в root-owned `/opt/nasa/config/.env`
> (это НЕ `~/nasa/config/.env`); после правки — `systemctl restart nasa-tunnel`.
> Актуальное состояние: [`docs/plans/SYSTEM_AUDIT_2026-08-10.md`](../plans/SYSTEM_AUDIT_2026-08-10.md).
