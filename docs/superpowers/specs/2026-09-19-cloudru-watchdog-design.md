...
VPS: nas-liveness (пользователь naswatch, без sudo)
   ├─ curl -m 10 127.0.0.1:18099/healthcheck   NAS API через обратный туннель
   ├─ curl -m 10 127.0.0.1:18080/status.php    Nextcloud через обратный туннель
...
| туннель открыт, сервис 5xx/таймаут 2 проверки подряд | 🟠 Nextcloud (или NAS API) отвечает ошибкой, туннель жив |
...
- Ключ сторожа: отдельный пользователь без sudo, одна команда. Утечка раскрывает только «жив ли NAS»
  и даёт отправку текста через VPS токеном, который злоумышленник принесёт сам.
...
- `nas-liveness`: `curl` подменён — переходы жив → не отвечает (1 раз: тишина) → тревога → повтор
