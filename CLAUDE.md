...
| Keenetic Omni KN-1410 | 📋 planned extender | Инвентаризован, в сеть не введён; см. `docs/25_KEENETIC_OMNI_KN1410.md` |
| TP-Link Deco E4 (2 шт.) | 📋 закуплен, не введён | План — `docs/26_DECO_E4_NETWORK.md`. 🔴 порты 100 Мбит/с → только режим AP, Jetson остаётся в гигабитном порту |
...
**🔜 Ближайшие задачи:**
- Обсудить топологию домашней сети (Deco E4) и ввод Vostro 15 как ML-узла — решение не принято.
...
| Keenetic Omni KN-1410 | IP pending | Planned Wi-Fi extender; factory reset только после явного подтверждения |
| TP-Link Deco E4 ×2 | не введён | Только режим Access Point; см. `docs/26_DECO_E4_NETWORK.md` |
| Vostro 15 ML-узел | `192.168.0.60` (план) | Dell Vostro 15 (2018, tag `H7YB9L2`) → always-on Immich ML-узел. См. `docs/plans/VOSTRO_ML_NODE_ONBOARDING.md` |
...
9. Keenetic Omni KN-1410 сначала подключать изолированно к workstation: не вводить в production LAN с активным DHCP и не делать factory reset без явного подтверждения.
10. Deco E4 вводить **только в режиме Access Point** — режим роутера меняет подсеть и ломает всю адресацию проекта.
