...
| Картинки (Kandinsky) | 🟠 **только генерация** | `/v1/image/generate` и `/v1/image/edit`. ⛔ **Реставрация/ретушь невозможны** — API не редактирует фото, см. `docs/plans/PHOTO_PROCESSING_FEASIBILITY.md` |
| `LLM_ALLOW_IMAGE_ANALYSIS` | 🟠 **true** на устройстве | Включено по явной просьбе владельца. Означает, что приложенные фото уходят провайдеру |
