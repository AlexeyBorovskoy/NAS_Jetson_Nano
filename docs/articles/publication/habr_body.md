...
Агент проанализировал 9 инструментов и объяснил, почему Zabbix не подойдёт на начальном этапе без дискового swap: нужна третья PostgreSQL и ещё 500+ MB RAM. Добавленные позднее 2 GB zram снижают риск пикового OOM, но не дают оснований утяжелять стек. Агент предложил лёгкую связку:
...
![Beszel Hub — обзор систем: Jetson Nano и VPS, оба онлайн](UPLOAD_1_beszel_systems_overview.png)

![Beszel Hub — метрики Jetson: CPU ~15%, RAM 2.3 GB, контейнеры](UPLOAD_2_beszel_jetson_metrics.png)
...
![Immich Android — статус бэкапа: 6697 из 6719 загружено](UPLOAD_3_android_immich_backup_stats.jpg)

Контакты синхронизировал через DAVx⁵ — 2 151 запись, CalDAV и CardDAV:

![DAVx⁵ — синхронизация CalDAV и CardDAV настроена](UPLOAD_4_android_davx5_caldav.jpg)
...
![Nextcloud Talk — групповой чат «Семья»](UPLOAD_5_nextcloud_talk.png)

![Nextcloud — дашборд с файлами, контактами и чатом](UPLOAD_6_nextcloud_dashboard.png)
...
![Immich — семейный фотоархив; исторический снимок на дату публикации (6.1 GiB, 228 GB свободно)](UPLOAD_8_immich_web.png)
...
- ML в Immich отключён — 2 GB zram помогают при пиковых нагрузках, но Jetson с 4 GB RAM всё равно не имеет достаточного запаса для распознавания лиц
